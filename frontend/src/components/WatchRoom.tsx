...
export const WatchRoom = () => {
    ...
    const handleJoin = (roomId: string, name: string) => {
        ...
        newSocket.on('play', ({ currentTime }) => {
            setRoomState(prev => prev ? { ...prev, playing: true, currentTime } : null);
        });

        newSocket.on('pause', ({ currentTime }) => {
            setRoomState(prev => prev ? { ...prev, playing: false, currentTime } : null);
        });
        ...
        newSocket.on('change_video', ({ videoId }) => {
            setRoomState(prev => prev ? { ...prev, videoId, currentTime: 0, playing: false } : null);
        });
    ...
    if (!joined) {
        return (
            <>
                {error && (
                    <div className="glass" style={{ position: 'fixed', top: '20px', left: '50%', transform: 'translateX(-50%)', padding: '12px 24px', color: 'var(--danger-color)', display: 'flex', alignItems: 'center', gap: '8px', zIndex: 100 }}>
                        <AlertCircle size={20} />
                        {error}
                    </div>
                )}
    ...
    const currentUser = roomState?.participants.find(p => p.userId === socket?.id);
    const canControl = currentUser?.role === 'host' || currentUser?.role === 'moderator';
    const isHost = currentUser?.role === 'host';

    return (
        <div className="container" style={{ height: '100vh', display: 'flex', flexDirection: 'column', padding: '20px', gap: '20px' }}>
            <header style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    ...
                    <div>
                        <h2 style={{ fontSize: '20px', fontWeight: '700' }}>{roomState?.roomId}</h2>
                        <p style={{ fontSize: '12px', color: 'var(--text-secondary)' }}>Live Watch Party</p>
                    </div>
                </div>
            ...
            <main style={{ flex: 1, display: 'grid', gridTemplateColumns: '1fr 320px', gap: '20px', minHeight: 0 }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                    <div className="glass" style={{ flex: 1, minHeight: 0, position: 'relative' }}>
                        {roomState && (
                            <Player
                    ...
                    {canControl && (
                        <form onSubmit={handleChangeVideo} className="glass" style={{ padding: '16px', display: 'flex', gap: '12px' }}>
                            <input
                                className="input"
