    ...
    return (
        <div className="glass" style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
            <div style={{ padding: '20px', borderBottom: '1px solid var(--glass-border)', display: 'flex', alignItems: 'center', gap: '12px' }}>
                ...
                <span style={{ marginLeft: 'auto', background: 'var(--glass-bg)', padding: '2px 8px', borderRadius: '12px', fontSize: '12px' }}>
                    {participants.length}
                </span>
            </div>
                ...
                {participants.map((p) => (
                    <div
                        ...
                        style={{
                            marginBottom: '8px',
                            padding: '12px',
                            border: p.userId === currentUserId ? '1px solid var(--primary-color)' : '1px solid var(--glass-border)',
                            display: 'flex',
                            alignItems: 'center',
                            gap: '12px'
                        }}
                    >
                        <div style={{ width: '32px', height: '32px', borderRadius: '50%', background: 'var(--glass-bg)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                            {p.role === 'host' ? <Shield size={16} color="#f59e0b" /> : <User size={16} />}
                        </div>

                        <div style={{ flex: 1 }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                <span style={{ fontWeight: '600', fontSize: '14px' }}>{p.username}</span>
                                {p.userId === currentUserId && <span style={{ fontSize: '10px', color: 'var(--primary-color)' }}>(You)</span>}
                            </div>
