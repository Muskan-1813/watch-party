...
app.use(
  cors({
    origin: ["https://watch-party-sandy.vercel.app", "http://localhost:5173"],
    methods: ["GET", "POST"],
  }),
);
...
const io = new Server(httpServer, {
  cors: {
    origin: ["https://watch-party-sandy.vercel.app", "http://localhost:5173"], // In production, limit to your frontend URL
    methods: ["GET", "POST"],
  },
});
...
io.on("connection", (socket) => {
  ...
  socket.on("join_room", ({ roomId, username }) => {
    ...
    console.log(
      `User ${username} joined room ${roomId} as ${participant.role}`,
    );
